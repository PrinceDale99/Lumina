/**
 * CLI for interacting with lumina-contract contract
 */
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log('║                   lumina-contract CLI                           ║');
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  try {
    ...
    while (running) {
      console.log('─── Menu ───────────────────────────────────────────────────────');
      console.log('  1. Verify Employee');
      console.log('  2. Read last verified wallet');
      ...
      switch (choice.trim()) {
        case '1': {
          const destinationWallet = await rl.question('  Enter destination wallet address: ');
          ...
          try {
            ...
            console.log(`\n  ✅ Verified employee for wallet: "${destinationWallet}"`);
        ...
        case '2': {
          ...
          try {
            ...
            if (contractState) {
              const ledgerState = Employee.ledger(contractState.data);
              const message = Buffer.from(ledgerState.lastVerifiedWallet).toString();
              console.log(`\n  📋 Last Verified Wallet: "${message}"\n`);
            } else {
